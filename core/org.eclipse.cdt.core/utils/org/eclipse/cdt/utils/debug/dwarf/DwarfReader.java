...
public class DwarfReader extends Dwarf implements ISymbolReader, ICompileOptionsFinder {
	...
	private void getCommandMacrosFromMacroSection() {
		...
		if (altdata != null) {
			while (altdata.hasRemaining()) {
				try {
					int original_position = altdata.position();
					int type = read_2_bytes(altdata);
					byte flags = altdata.get();
					boolean offset_size_8;
					long lt_offset = -1;
					String fileName = null;
					...
					if (DEBUG)
						System.out.println("alt type is " + type); //$NON-NLS-1$

					// bottom bit 0 tells us whether we have 8 byte offsets or 4 byte offsets
					offset_size_8 = (flags & 0x1) == 1;
					if (DEBUG)
						System.out.println("alt offset size is " + (offset_size_8 ? 8 : 4)); //$NON-NLS-1$
					// bit 1 indicates we have an offset from the start of .debug_line section
					if ((flags & 0x2) != 0) {
						lt_offset = (offset_size_8 ? read_8_bytes(altdata) : read_4_bytes(altdata));
						fileName = m_stmtFileMap.get(Long.valueOf(lt_offset));
						if (DEBUG)
							System.out.println("debug line offset is " + lt_offset); //$NON-NLS-1$
					}
					...
					if ((flags & 0x4) != 0) {
						opcodeInfos = new HashMap<Integer, OpcodeInfo>();
						int num_opcodes = altdata.get();
						for (int i = 0; i < num_opcodes; ++i) {
							OpcodeInfo info = new OpcodeInfo(offset_size_8);

							int opcode = altdata.get();
							long numArgs = read_unsigned_leb128(altdata);
							info.setNumArgs((int)numArgs);
							for (int j = 0; j < numArgs; ++j) {
								int argType = altdata.get();
								info.addArgType(argType);
							}
							opcodeInfos.put(Integer.valueOf(opcode), info);
						}
					...
					while (!done) {
						...
						case DwarfConstants.DW_MACRO_start_file: {
							long filenum;
							long lineno;
							lineno = read_signed_leb128(altdata);
							filenum = read_signed_leb128(altdata);
							// All command line macros are defined as being included before start of file
							if (filenum == 1 && lt_offset >= 0) {
								// we have a source file so add all macros defined before it with lineno 0
								m_compileOptionsMap.put(fileName, macros);
						...
						case DwarfConstants.DW_MACRO_define_indirect: {
							...
							long offset;
							lineno = read_signed_leb128(altdata);
							offset = (offset_size_8 ? read_8_bytes(altdata) : read_4_bytes(altdata));
							// Within alt macro section, indirect references are within the alt string section.
							altstr.position((int)offset);
							String macro = readString(altstr);
							if (lineno == 0)
								macros.add(getCommandLineMacro(macro));
						...
						break;
						case DwarfConstants.DW_MACRO_define_indirect_alt: { // shouldn't see this
							long lineno;
							long offset;
							lineno = read_signed_leb128(altdata);
							offset = (offset_size_8 ? read_8_bytes(altdata) : read_4_bytes(altdata));
						...
						break;
						case DwarfConstants.DW_MACRO_undef_indirect_alt: { // shouldn't see this
							long lineno;
							long offset;
							String macro;
							lineno = read_signed_leb128(altdata);
						...
						case DwarfConstants.DW_MACRO_transparent_include: {
							long offset;
							offset = (offset_size_8 ? read_8_bytes(altdata) : read_4_bytes(altdata));
							// Within alt macro section, indirect references are within the alt macro section.
							ArrayList<String> foundMacros = t_alt_macros.get(Long.valueOf(offset));
							if (foundMacros != null)
								macros.addAll(foundMacros);
							else if (lt_offset >= 0) {
								macros.add("=FIXUPALT=" + offset); // leave a marker we can fix up later //$NON-NLS-1$
						...
						case DwarfConstants.DW_MACRO_end: {
							if (lt_offset < 0) {
								if (DEBUG)
									System.out.println("creating transparent include macros for offset: " + original_position); //$NON-NLS-1$
								t_alt_macros.put(Long.valueOf(original_position), macros);
							}
							done = true;
						}
						break;
		...
		for (String name: fixupAltList) {
			ArrayList<String> macros = m_compileOptionsMap.get(name);
			for (int i = 0; i < macros.size(); ++i) {
				String macroLine = macros.get(i);
				if (macroLine.startsWith("=FIXUPALT=")) { //$NON-NLS-1$
					Long offset = Long.valueOf(macroLine.substring(7));
					if (DEBUG)
						System.out.println("Found alt fixup needed for offset: " + offset + " for file: " + name); //$NON-NLS-1$ //$NON-NLS-2$
					ArrayList<String> insertMacros = t_alt_macros.get(offset);
					if (DEBUG)
						System.out.println("alt insert macros are: " + insertMacros.toString()); //$NON-NLS-1$
					macros.remove(i);
					macros.addAll(i, insertMacros);
					i += insertMacros.size();
				}
